...
*   **Frontend**: Vanilla HTML/JS/CSS (`index.html`, `dashboard.html`, `login.html`). No modern frameworks. Uses `sessionStorage` for JWT tokens.
...
*   **Database**: Flat JSON files (e.g., `users.json` for auth, `data/projects_<email>.json` for project data).

## 2. Key Files & Responsibilities
*   **`server.js` (800+ lines)**: The central backend file.
    *   Auth routes (`/api/login`, `/api/me`) using JWT and `bcryptjs`.
...
3.  **`GeneticAlgorithm.run()`**: Evolves solutions until `conflicts === 0` (or it hits a hard safety timeout of 25 seconds).
